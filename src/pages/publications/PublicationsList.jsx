// src/pages/publications/PublicationsList.jsx
import { useState, useEffect } from 'react'
import { Link } from 'react-router-dom'
import { PlusCircle, Pencil, Trash2, Search, RefreshCw } from 'lucide-react'
import { getPublications, deletePublication } from '../../api/publications'
...
export default function PublicationsList() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await getPublications({ page, per_page: 15, search: search || undefined })
      setData(res)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="animate-fade-in">
      <Header
        title="Publications"
        subtitle={`${data.total} publication(s) au total`}
        actions={
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-slate-500 text-sm">Aucune publication trouvée</p>
            <Link to="/publications/new" className="btn-primary mt-4 inline-flex">
              <PlusCircle className="w-4 h-4" />
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-surface-200/10">
              {data.items.map((pub) => (
                <tr key={pub.id} className="hover:bg-surface-800/50 transition-colors duration-150 group">
                  <td className="px-5 py-4">
                    <p className="text-sm font-medium text-white line-clamp-1">{pub.title}</p>
                    {pub.excerpt && (
                      <p className="text-xs text-slate-500 mt-0.5 line-clamp-1">{pub.excerpt}</p>
                    )}
                  </td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    {pub.category
                      ? <span className="text-xs text-slate-400 bg-surface-800 px-2.5 py-1 rounded-lg">{pub.category.name}</span>
                      : <span className="text-xs text-slate-600">—</span>
                    }
                  </td>
                  <td className="px-5 py-4 hidden lg:table-cell">
                    <span className="text-xs text-slate-500 font-mono">{formatDate(pub.published_at || pub.created_at)}</span>
                  </td>
                  <td className="px-5 py-4">
